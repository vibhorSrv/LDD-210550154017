/**
 * Character driver thet performs multiplication operation
 */
...
MODULE_DESCRIPTION("Character driver thet performs MUL operation");
...
static int __init MUL_MOD_init(void)
{
    int result, MAJ, MIN;
    dev_t my_device;

    INFO("MUL_MOD_init() enter");
    ...
    INFO("MUL_MOD_init() exit");

    return 0;
}

static void __exit MUL_MOD_exit(void)
{
    int MAJ, MIN;
    dev_t my_device = MKDEV(DEV_MAJ, DEV_MIN);
    INFO("MUL_MOD_exit() enter")
    ...
    INFO("MUL_MOD_exit() exit");
}

module_init(MUL_MOD_init);
module_exit(MUL_MOD_exit);
