/**
 * Character driver thet performs add operation
 */
...
static int __init ADD_MOD_init(void)
{
    int result, MAJ, MIN;
    dev_t my_device;

    INFO("ADD_MOD_init() enter");
    ...
    INFO("ADD_MOD_init() exit");

    return 0;
}

static void __exit ADD_MOD_exit(void)
{
    int MAJ, MIN;
    dev_t my_device = MKDEV(DEV_MAJ, DEV_MIN);
    INFO("ADD_MOD_exit() enter")
    ...
    INFO("ADD_MOD_exit() exit");
}

module_init(ADD_MOD_init);
module_exit(ADD_MOD_exit);
 ...
 * 
 */
ssize_t ADD_DEV_write(struct file *__file, const char *__ubuf, size_t __nbytes, loff_t *offp)
{
    int result, bytes_to_write;
    ssize_t retval;

    INFO("ADD_DEV_write(fp=%p,ubuf=[%s],nbytes=%d)", __file, __ubuf, __nbytes);
 ...
 * 
 */
ssize_t ADD_DEV_read(struct file *__file, char *__ubuf, size_t __nbytes, loff_t *offp)
{
    int result, oper_result;
    ssize_t retval;
    char oper_result_string[30] = {'\0'};

    INFO("ADD_DEV_read(fp=%p,ubuf=[%s],nbytes=%d)", __file, __ubuf, __nbytes);
