/**
 * Character driver that performs division operation
 */
...
MODULE_DESCRIPTION("Character driver thet performs DIV operation");
...
static int __init DIV_MOD_init(void)
{
    int result, MAJ, MIN;
    dev_t my_device;

    INFO("DIV_MOD_init() enter");
    ...
    INFO("DIV_MOD_init() exit");

    return 0;
}

static void __exit DIV_MOD_exit(void)
{
    int MAJ, MIN;
    dev_t my_device = MKDEV(DEV_MAJ, DEV_MIN);
    INFO("DIV_MOD_exit() enter")
    ...
    INFO("DIV_MOD_exit() exit");
}

module_init(DIV_MOD_init);
module_exit(DIV_MOD_exit);
